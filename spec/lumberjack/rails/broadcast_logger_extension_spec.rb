...
RSpec.describe Lumberjack::Rails::BroadcastLoggerExtension do
  ...
  let(:other_lumberjack_logger) { Lumberjack::Logger.new(:test, template: ":message") }
  ...
  it "can tag logs in the debug method" do
    ...
    expect(out.string).to include("Debug message")
  ...
  it "can tag logs in the info method" do
    ...
    expect(out.string).to include("Info message")
  ...
  it "can tag logs in the warn method" do
    ...
    expect(out.string).to include("Warn message")
  ...
  it "can tag logs in the error method" do
    ...
    expect(out.string).to include("Error message")
  ...
  it "can tag logs in the fatal method" do
    ...
    expect(out.string).to include("Fatal message")
  ...
  it "can tag logs in the unknown method" do
    ...
    expect(out.string).to include("Unknown message")
  ...
  describe "multiple yield protection" do
    ...
    before do
      broadcast_logger.broadcasts << other_lumberjack_logger
    end

    it "does not allow multiple lumberjack loggers in the initializer" do
      expect {
        ActiveSupport::BroadcastLogger.new(logger, other_lumberjack_logger)
    ...
    it "does not yield multiple times when calling log_at and does not log any warnings" do
      ...
      expect(other_lumberjack_logger.device.entries).to be_empty
    end
    ...
    it "does not yield multiple when calling tag with a block and logs warnings" do
      ...
      expect(other_lumberjack_logger.device).to include(severity: :warn, message: /It was not called on this logger/)
    end
